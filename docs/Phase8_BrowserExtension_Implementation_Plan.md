...
| Chat History Archive (Phase 5), Files (Phase 6) | **Updated since this table was first written**: both shipped in code after this document's original draft (`import.service`/`chat-search.service`, `file.service`/`rag.service`, both bucket-scoped exactly as this plan assumed they would be). Neither changes anything below — this document's extension surface still only calls the Phase 2/3/4 endpoints in §5.3; a "recall from Chat History/Files" inject mode is still deferred to Phase 7 (next row), same reasoning as before, just confirming it wasn't blocked on these two |
| Ask (Phase 7) | **Still does not exist in code** — only a planning gap remains, filled by the `Phase7_Implementation_Plan.md` written alongside this update. Any extension surface depending on it (an "Ask this bucket" inject mode) stays explicitly deferred, and this plan's architecture is written so adding it later is additive (§9), not a rebuild |
